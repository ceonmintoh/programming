...
class inputWindows():
    ...
    def addOrderWidget():
        #frame.destroy()
        addWorker = ttk.Frame(window)
        lb1 = ttk.Label(addWorker, text="Brand Name").grid(row=0, column=0, pady=4)
        lb2 = ttk.Label(addWorker, text="Brand Size").grid(row=1, column=0, pady=4)
        lb3 = ttk.Label(addWorker, text="Variance Name").grid(row=2, column=0, pady=4)
        lb4 = ttk.Label(addWorker, text="Unit Price").grid(row=3, column=0, pady=4)
        lb5 = ttk.Label(addWorker, text="Buyer Name").grid(row=4, column=0, pady=4)
        lb6 = ttk.Label(addWorker, text="Quantity").grid(row=5, column=0, pady=4)
        lb7 = ttk.Label(addWorker, text="Total Price").grid(row=0, column=0, pady=4)
        submit_order = ttk.Button(addWorker, text='Create Order', command='insertUser').grid(row=7, column=0, pady=4)
        submit_Print = ttk.Button(addWorker, text='Print Invoice', command='insertUser').grid(row=7, column=1, pady=4)

        uname = ttk.Combobox(addWorker).grid(row=0, column=1, pady=4)
        fname = ttk.Combobox(addWorker).grid(row=1, column=1, pady=4)
        lname = ttk.Combobox(addWorker).grid(row=2, column=1, pady=4)
        address = ttk.Combobox(addWorker).grid(row=3, column=1, pady=4)
        dob = ttk.Entry(addWorker).grid(row=4, column=1, pady=4)
        phone = ttk.Entry(addWorker).grid(row=5, column=1, pady=4)
        email = ttk.Entry(addWorker).grid(row=6, column=1, pady=4)
